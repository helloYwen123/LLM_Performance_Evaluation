...
class IterativeMergeConclusion:
    def __init__(self,
        ...
        self.self_reflect_completion_start = self_self_reflect_completion_start
    ...
    def _execute(self, 
                ...
                api: Optional['API'],
                target: Optional['Node']) -> Dict[str, Any]:
                
        # Get all answerable_nodes through graph.get_concludable_nodes(). If ranking_confidence isn't None, append these nodes into answerable_nodes list
        answerable_nodes = [n for n in graph.get_concludable_nodes() if n.state.ranking_confidence]

        # Use root's spatial_node_state action captions summary to obtain whole_video_summary & use root's state.task to retrieve variable task
        whole_video_summary = graph.root.state.spatial_node_state.action_captions_summary
        task = graph.root.state.task
        
        # Create an empty list to store answerable_nodes' shallow copies sorted based on their ranking_confidences
        answerable_nodes_list = []
        for idx, node in enumerate(answerable_nodes):            
            answerable_nodes_list.append({
                'index': idx,
                'node': node,
                'ranking_confidence': node.state.ranking_confidence
            })
        
        # Sort answerable_nodes according to descending order of ranking_confidence values
        answerable_nodes_list.sort(key=lambda x: -x['ranking_confidence'])
        
        # Initialize answerable_concatenated_summaries with {}
        answerable_concatenated_summaries = {}

        # Iterate each node within answerable_nodes_list starting at first index until second-to-last index
        for i, item in enumerate(answerable_nodes_list[:-1]):
            start_index = item['node'].state.video_clip.sampled_indices[0]
            end_index = item['node'].state.video_clip.sampled_indices[-1]
            interval_center = sum(item['node'].state.video_clip.sampled_indices)/len(item['node'].state.video_clip.sampled_indices)
            answerable_concatenated_summaries[interval_center] = item['node'].state.get_lexical_representation()

        # Join concatenated summaries separated by "\n"
        joined_summaries = '\n'.join([str(v) for k,v in answerable_concatenated_summaries.items()])
        
        # Check whether options value contains N/A; if so, return intermediate_candidate directly setting initial_value as final_prediction
        if all(option == 'N/A' for option in task.options.values()):
            intermediate_candidate = ['initial_value']
            final_prediction = 'initial_value'
        else:
            intermediate_candidate = self.derive_options_candidate_from_whole_video_state(whole_video_summary)

            try:
                final_prediction = intermediate_candidate[0]['prediction']
            except KeyError:
                final_prediction = 'failed to parse prediction from completion'

        # Create a dictionary containing various information such as concatenation lexical states etc...
        conclusion = {
            "concatenated_lexical_states": answerable_concatenated_summaries,
            "completion": intermediate_candidate[1],
            "final_ranking": intermediate_candidate[0],
            "final_prediction": final_prediction
        }

        # Log debug messages about the process
        logger.debug("Whole video state (i=%r): %r", i, answerable_concatenated_summaries)
        logger.debug("Completion (i=%r): %r", i, intermediate_candidate[1])
        logger.debug("Predicted answer (i=%r): %r", i, intermediate_candidate[0])

        # Store results under corresponding keys inside conclusions dictionary
        conclusions = [{"node_interval_center": interval_center, "conclusion": concl} for interval_center, concl in answerable_concatenated_summaries.items()]

        # Replace "{reasoning_history}" placeholders in self_reflect_prompt with actual reasoning history obtained earlier
        formatted_self_reflect_prompt = self_reflect_prompt.replace("{reasoning_history}", reasoning_history).format(reasoning_history=reasing_history)

        # Call API to fetch completion response given updated self_reflect_prompt
        completion_response = api.get_completion_from_text(formatted_self_reflect_prompt)

        # Parse JSON response received back from API call
        parsed_data = self.parse_answer_json(completion_response.text, keywords_in_priority_order=self.keywords_in_priority_order, candidate_function=self.get_single_number_candidates_from_text)

        # Set confidence score between 1 and 3 inclusive
        confidence_score = max(min(parsed_data["confidence"], 3), 1)

        # Break out early when reaching maximum allowed confidence threshold
        if confidence_score >= 3:
            break
            
        # Generate comprehensive conclusion dictionary encompassing number iterations performed alongwith individual iteration details etc..
        conclusion_dict = {
            "number_of_iterations": len(conclusions),
            "iteration_conclusions": conclusions,
            "whole_video_state": answerable_concatenated_summaries,
            "completion": list(conclusions)[-1]["conclusion"]["completion"],
            "final_ranking": list(conclusions)[-1]["conclusion"]["final_ranking"],
            "final_prediction": list(conclusions)[-1]["conclusion"]["final_prediction"]
        }
...
# \n\n Now i will give you the part of this class which i have finished, help me to complete it based on the above requirment and description and return me the completed code in final.
# \nThe partly code that you need to complete is following: 
...
class IterativeMergeConclusion(Operation):
    ...
    def _execute(self, graph: Optional[Graph], api: Optional[API], target: Optional[Node]) -> dict[str, any]:
        ...
        answerable_nodes = [
            node for node in graph.get_concludable_nodes() if node.state.ranking_confidence is not None]
        ...
        for i, node in enumerate(answerable_nodes):
            ...
            # Experimentally tested with Llama 3 that there's about 3% performance increase when collecting
            # all nodes with Level 3 ranking_confidence before performing the initial merge and inference
            if i != len(answerable_nodes) - 1:
                # Continue merging until reaching another node at rank confidencce < 3
                while True:
                    if answerable_nodes[i+1].state.ranking_confidence >= 3:
                        i += 1
                    else:
            ...
            # Otherwise, merge the lexical states of all current nodes in their temporal order
            # Sort the answerable_concatenated_summaries by the center of the interval
            answerable_concatenated_summaries = dict(sorted(answerable_concatenated_summaries.items()))
            ...
            # Note that our framework currently supports Free Form Questions & Single Choice Questions
            # Decide which candidate derivation function to use based on Availability of The Task Options
            # Check If All Options Are "N/A", I.E., Not Available
            if all(option == "N/A" for option in task.options.values()):
                # Get the Free Form QA Prediction
...
# \n\n Now i will give you the part of needed class with many blanks which i have finished, help me to fill and complete it based on the following requirment and description. Give me the completed code finally.
# \n The following is the imcomplete code with many blanks which you need to finish:
# \n class IterativeMergeConclusion(Operation):
# def __init__(
#         self,
#         qa_prompt: str,
...
#     self.qa_prompt = qa_prompt
#     self. = qa_completion_start
#     self.qa_max_new_tokens = qa_max_new_tokens
#     self.qa_temperature = qa_temperature
#     self.qa_replace_c = 
#     self.qa_parse_strategy = qa_parse_strategy

#     self. = self_reflect_prompt
#     self.self_reflect_completion_start = self_reflect_completion_start
#     self.self_reflect_max_new_tokens = 
#     self.self_reflect_temperature = self_reflect_temperature
#     self. = self_reflect_parse_strategy
...
#     # now we can change the order of the new list without affecting the order of the original list
#     # specifically, we want to sort the nodes by their ranking confidence (from highest to lowest)
#     answerable_nodes.sort(key=lambda x: x.state., reverse=True)
...
#         # get the center point of the interval of the node
#          = node.state.video_clip.sampled_indices[0]
#         interval_end = node.state.video_clip.sampled_indices[-1]
#         interval_center = (interval_start + ) / 2
...
#         # otherwise we will merge the lexical states of all current nodes in their temporal order
#         # sort the answerable_concatenated_summaries by the center of the interval
#          = dict(sorted(answerable_concatenated_summaries.items()))
...
#             final_prediction = [0][0] if intermediate_candidate[
#                 0] else "failed to parse prediction from completion"
...
#         reasoning_history = .replace("{", "{{").replace("}", "}}")
...
#         "iteration_conclusions": conclusions,
#         "whole_video_state": ,
#         "": list(conclusions.values())[-1]["conclusion"]["completion"],
#         "final_ranking": list(conclusions.values())[-1]["conclusion"]["final_ranking"],
#         "final_prediction": list(conclusions.values())[-1]["conclusion"]["final_prediction"],
#     }

#     logger.info(f"Executed llm-based conclusion operation: "
#                 f" -> {conclusion['final_prediction']}")
...
class IterativeMergeConclusion(Operation):
    ...
    def _execute(self, graph: Optional[Graph], api: Optional[API], target=None) -> dict[str, any]:
        answerable_nodes = [node for node in graph.get_concludable_nodes() if node.state.ranking_confidence is not None]
        whole_video_summary = graph.root.state.spatial_node_state.action_captions_summary
        task = graph.root.state.task
        ...
        for i, node in enumerate(answerable_nodes):
            ...
            if all(option == "N/A" for option in task.options.values()):
                ...
                final_prediction = [0][0] if intermediate_candidate[0] else "failed to parse prediction from completion"
            ...
            # build the self-reflection prompt for the self-reflection
            self_reflection_prompt = self.self_reflect_prompt.format(reasoning_history=reasoning_history)
            ...
            # parse the answerability from the completion
            # define the priority of certain keywords
            keywords_in_priority_order = ['confidence', 'level', 'conf', 'answerability', 'confidence_level', ' confidence']
            candidate = parse_answer_json(text=completion, keywords_in_priority_order=keywords_in_priority_order, candidate_fun=get_single_number_candidates_from_text)
        ...
        logger.info(f"Executed llm-based conclusion operation:"
                    f"\n\t-> Final Prediction: {conclusion['final_prediction']}"
                   )
